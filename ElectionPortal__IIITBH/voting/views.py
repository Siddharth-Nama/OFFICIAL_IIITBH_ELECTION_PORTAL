            ...
            candidates_data += f"""
                <div class="candidate-item">
                    <div class="candidate-content">
                        {input_box}
                        <img src="{image}" height="100px" width="100px" class="clist"><br>
                        <span class="cname clist">{candidate.fullname}</span><br>
                        <button type="button" class="btn btn-primary btn-sm btn-flat clist platform" 
                            ...
                            <i class="fa fa-search"></i> View Bio
                        </button>
                    </div>
                </div>
            """
...
def submit_ballot(request):
    ...
    postsnum = Position.objects.all().count()
    
    # Ensure at least one vote is selected
    if len(form.keys()) < postsnum:
        messages.error(request, "Please click Next button to vote for all other Positions")
        return redirect(reverse('show_ballot'))
    positions = Position.objects.all()
    form_count = 0
    for position in positions:
